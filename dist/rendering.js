...
import { sourceGridData, currentGridData, isInterpolationEnabled, interpolationSteps, saturationOffset, selectedCells, setCurrentGridData, } from "./state.js";
import { isValidHex, hexToRgb, rgbToHex, adjustSaturation, getTextColor, parseXValue, interpolateRgb, } from "./colorUtils.js";
...
export function generateInterpolatedPalette(sourceData, steps) {
    ...
    sourceData.forEach((row, rowIndex) => {
        ...
        if (rowIndex === sourceData.length - 1 || steps === 0)
            return;
        ...
        for (let step = 1; step <= steps; step++) {
            ...
            row.forEach((cell, cellIndex) => {
                ...
                if (currentX !== null && nextX !== null) {
                    const interpolatedX = Math.round(currentX + (nextX - currentX) * factor);
...
export function updatePaletteView() {
    if (isInterpolationEnabled) {
        setCurrentGridData(generateInterpolatedPalette(sourceGridData, interpolationSteps));
    }
    else {
...
export function applySelectionStyles(coords) {
    ...
    const cellDiv = paletteGrid?.querySelector(`.cell-content[data-row-index="${rowIndex}"][data-cell-index="${cellIndex}"]`);
    if (cellDiv) {
...
export function clearSelectionVisuals() {
    ...
    selectedCells.forEach((coord) => {
        const cellDiv = paletteGrid?.querySelector(`.cell-content[data-row-index="${coord.row}"][data-cell-index="${coord.col}"]`);
        if (cellDiv) {
    ...
    gridData.forEach((rowData, rowIndex) => {
        ...
        rowData.forEach((cellData, cellIndex) => {
            ...
            if (isValidHex(cellData)) {
                ...
                renderLabelCell(cellContentDiv, cellData, isInterpolatedLabelPlaceholder, cellIndex, rowData, rowIndex, gridData);
            }
            else {
...
function renderSwatchCell(cellContentDiv, cellData, rowIndex, cellIndex) {
    ...
    if (selectedCells.some((coord) => coord.row === rowIndex && coord.col === cellIndex)) {
...
// Render a label cell
function renderLabelCell(cellContentDiv, cellData, isInterpolatedLabelPlaceholder, cellIndex, rowData, rowIndex, gridData) {
