...
import { sourceGridData, isPanning, pointerHasMoved, pointerDownPos, touchEventHandled, isColorPickingMode, isInterpolationEnabled, selectedCells, selectionAnchor, isAwaitingRangeEndTap, paletteOffsetX, paletteOffsetY, scale, startX, startY, currentX, currentY, setPanning, setPanCoords, setPaletteOffset, setPointerState, setTouchEventHandled, setSelectedCells, setSelectionAnchor, setAwaitingRangeEndTap, clearSelection, } from "./state.js";
import { isValidHex, hexToHsl } from "./colorUtils.js";
...
export function startPan(event) {
    ...
    const targetIsSwatch = target.classList.contains("cell-content") &&
        target.classList.contains("swatch");
    if (target.closest("#popout-editor") ||
...
export function handleSwatchSelection(event, rowIndex, colIndex, clickedHsl) {
    const isMultiSelectIntent = (event.type === "touchend" &&
        ...
        (event.type === "click" &&
            ...
            event.shiftKey &&
            !event.ctrlKey);
    const existingIndex = selectedCells.findIndex((coord) => coord.row === rowIndex && coord.col === colIndex);
    ...
    if (isMultiSelectIntent) {
        if (existingIndex > -1) {
            ...
            if (selectedCells.length === 0) {
                ...
                if (firstSelectedCoords.row >= sourceGridData.length ||
                ...
                const firstSelectedHex = sourceGridData[firstSelectedCoords.row][firstSelectedCoords.col];
                ...
                if (firstSelectedHsl) {
                    ...
                    if (sDiff <= CONSTANTS.MULTI_SELECT_TOLERANCE ||
...
export function setupSwatchEventListeners(cellContentDiv, originalHexColor, _rowIndex, _cellIndex) {
    // Tooltip listeners
    ...
    if (CONSTANTS.IS_TOUCH_DEVICE) {
        cellContentDiv.addEventListener("touchend", (event) => {
            ...
            if (touchRowIndex >= sourceGridData.length ||
    ...
    cellContentDiv.addEventListener("click", (event) => {
        ...
        if (clickRowIndex >= sourceGridData.length ||
        ...
        if (clickedHsl) {
            handleDesktopClick(event, clickRowIndex, clickCellIndex, clickedHsl, cellContentDiv);
        }
        setTouchEventHandled(false);
    });
    // Hover effects
...
function handleTouchRangeSelection(touchRowIndex, touchCellIndex, event) {
    if (isAwaitingRangeEndTap) {
        ...
        else if (selectionAnchor.row === touchRowIndex &&
...
function handleDesktopClick(event, clickRowIndex, clickCellIndex, clickedHsl, cellContentDiv) {
...
function openColorPicker(row, col, _event) {
