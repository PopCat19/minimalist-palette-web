...
let popoutHeader: HTMLElement | null;
let popoutCloseButton: HTMLElement | null;
let popoutPaletteInput: HTMLTextAreaElement | null;
let popoutUpdateButton: HTMLElement | null;
let popoutResizeHandle: HTMLElement | null;
let popoutStatusMessage: HTMLElement | null;

// Dragging State
let isDraggingPopout: boolean = false;
let popoutStartX: number,
  popoutStartY: number,
  popoutInitialX: number,
  popoutInitialY: number;

// Resizing State
let isResizingPopout: boolean = false;
...
const SNAP_THRESHOLD: number = 20;
const SNAP_GAP: number = 16;

// Status message timeout
let statusTimeout: ReturnType<typeof setTimeout> | null = null;

// Initialize popout editor
export function initPopoutEditor(): void {
  // Get DOM elements
...
function setupPopoutEventListeners(): void {
  // Close button
...
export function openPopoutEditor(): void {
  if (!popoutEditor || !popoutPaletteInput) return;
...
export function closePopoutEditor(): void {
  if (popoutEditor) {
...
function handlePopoutUpdate(): void {
  if (!popoutPaletteInput || !popoutUpdateButton) return;
...
export function showPopoutStatus(
  message: string,
  isError: boolean = false,
): void {
  if (!popoutStatusMessage) return;
...
// Drag functionality
function startPopoutDrag(event: MouseEvent | TouchEvent): void {
  ...
  if (!popoutEditor || !popoutHeader) return;
...
function dragPopout(event: MouseEvent | TouchEvent): void {
  if (!isDraggingPopout || !popoutEditor) return;
  ...
  potentialLeft = Math.max(
    0,
    Math.min(potentialLeft, viewportWidth - editorWidth),
  );
  potentialTop = Math.max(
    0,
    Math.min(potentialTop, viewportHeight - editorHeight),
  );
...
function startPopoutResize(event: MouseEvent | TouchEvent): void {
  // Prevent resize if target is not the handle
  if (event.target !== popoutResizeHandle) return;
...
function resizePopout(event: MouseEvent | TouchEvent): void {
  if (!isResizingPopout || !popoutEditor) return;
...
function stopPopoutResize(): void {
  if (isResizingPopout) {
...
export function restorePopoutState(state: PopoutState | null): void {
  if (!popoutEditor || !state) return;

  if (state.left) popoutEditor.style.left = state.left;
  if (state.top) popoutEditor.style.top = state.top;
  if (state.width) popoutEditor.style.width = state.width;
  if (state.height) popoutEditor.style.height = state.height;
}

// Check if dragging is active (for preventing other interactions)
export function isPopoutDragging(): boolean {
  return isDraggingPopout;
}

// Check if resizing is active (for preventing other interactions)
export function isPopoutResizing(): boolean {
