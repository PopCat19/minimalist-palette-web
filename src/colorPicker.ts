...
import { isValidHex, hexToHsl, hslToHex, HSL } from "./colorUtils.js";
...
// DOM Elements
let colorPickerModal: HTMLElement | null;
let colorPickerCloseButton: HTMLElement | null;
let colorPickerPreview: HTMLElement | null;
let pickerHueSlider: HTMLInputElement | null;
let pickerHueNumber: HTMLInputElement | null;
let pickerSatSlider: HTMLInputElement | null;
let pickerSatNumber: HTMLInputElement | null;
let pickerLumSlider: HTMLInputElement | null;
let pickerLumNumber: HTMLInputElement | null;
let pickerHexInput: HTMLInputElement | null;
let pickerCopyHexButton: HTMLElement | null;
let pickerCancelButton: HTMLElement | null;
let pickerApplyButton: HTMLElement | null;
let colorPickerHeader: HTMLElement | null;
let colorPickerResizeHandle: HTMLElement | null;

// Drag/Resize State
let isDraggingColorPicker: boolean = false;
let pickerStartX: number,
  pickerStartY: number,
  pickerInitialX: number,
  pickerInitialY: number;
let isResizingColorPicker: boolean = false;
let cpResizeStartX: number,
  cpResizeStartY: number,
  cpResizeInitialWidth: number,
  cpResizeInitialHeight: number;

// Constants
const multiSelectTolerance: number = 2;
...
function setupColorPickerEventListeners(): void {
  // Close button
  ...
  if (colorPickerResizeHandle) {
    colorPickerResizeHandle.addEventListener(
      "mousedown",
      startColorPickerResize,
    );
    colorPickerResizeHandle.addEventListener(
      "touchstart",
      startColorPickerResize,
...
export function openColorPicker(
  ...
  if (!isValidHex(sourceHex)) {
    console.error(
      `Invalid hex color at sourceGridData[${rowIndex}][${cellIndex}]: ${sourceHex}`,
    );
    clearSelection();
    return;
  }
...
function updatePickerPreview(hexColor: string): void {
...
function updatePickerFromHsl(): void {
  if (
    isPickerUpdating ||
    !pickerHueSlider ||
    !pickerSatSlider ||
...
function updatePickerFromHex(): void {
  if (isPickerUpdating || !pickerHexInput) return;
...
// Apply color changes
function applyColorChanges(): void {
  if (!pickerHexInput) return;
...
function applyMultiSelectChanges(newHex: string): void {
  const newHsl = hexToHsl(newHex);
  if (!newHsl) return;

  const isSimilarSelection = checkSimilarSelection();

  selectedCells.forEach((coord: CellPosition) => {
    if (
      coord.row < sourceGridData.length &&
      ...
      if (isSimilarSelection) {
        ...
        if (oldHsl) {
          const updatedHsl: HSL = { h: newHsl.h, s: oldHsl.s, l: oldHsl.l };
...
function checkSimilarSelection(): boolean {
  if (selectedCells.length <= 1) return false;

  const firstCoord = selectedCells[0];
  if (
    firstCoord.row >= sourceGridData.length ||
  ...
  return selectedCells.every((coord: CellPosition) => {
    if (
      coord.row >= sourceGridData.length ||
      coord.col >= sourceGridData[coord.row].length
    ) {
...
function positionColorPicker(event: MouseEvent | TouchEvent | null): void {
  if (!colorPickerModal) return;
...
function startColorPickerDrag(event: MouseEvent | TouchEvent): void {
  if (!colorPickerModal) return;
...
// Drag color picker
function dragColorPicker(event: MouseEvent | TouchEvent): void {
  if (!isDraggingColorPicker || !colorPickerModal) return;
...
function stopColorPickerDrag(): void {
...
function startColorPickerResize(event: MouseEvent | TouchEvent): void {
  if (!colorPickerModal) return;
...
// Resize color picker
function resizeColorPicker(event: MouseEvent | TouchEvent): void {
  if (!isResizingColorPicker || !colorPickerModal) return;
...
function stopColorPickerResize(): void {
