...
import { parseSimpleFormat, convertToSimpleFormat } from './colorUtils.js';
...
export function initPopoutEditor() {
    ...
    popoutHeader = popoutEditor?.querySelector('.popout-header');
...
function setupPopoutEventListeners() {
    ...
    if (popoutHeader) {
        ...
        popoutHeader.addEventListener('touchstart', startPopoutDrag, { passive: false });
    }

    // Resize listeners
    if (popoutResizeHandle) {
        popoutResizeHandle.addEventListener('mousedown', startPopoutResize);
        popoutResizeHandle.addEventListener('touchstart', startPopoutResize, { passive: false });
    }
}
