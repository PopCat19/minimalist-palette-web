...
// Store current cell element for tooltip operations
let currentCellElement: HTMLElement | null = null;

// Update and show palette tooltip
export function updateAndShowPaletteTooltip(
  hexColor: string,
  event: MouseEvent,
  cellElement: HTMLElement,
): void {
  const paletteTooltip = document.getElementById("palette-tooltip");
  const tooltipHexValue = document.querySelector(
    "#palette-tooltip #tooltip-hex span",
  ) as HTMLElement | null;
  const tooltipHslValue = document.querySelector(
    "#palette-tooltip #tooltip-hsl span",
  ) as HTMLElement | null;
  const tooltipRgbValue = document.querySelector(
  ...
  setCurrentTooltipTargetCell({ row: 0, col: 0 });
...
export function hidePaletteTooltip(): void {
  const paletteTooltip = document.getElementById("palette-tooltip");
...
export function handleTooltipCopy(event: Event): void {
  const tooltipHexValue = document.querySelector(
    "#palette-tooltip #tooltip-hex span",
  ) as HTMLElement | null;
  const tooltipHslValue = document.querySelector(
  ...
  if (span === tooltipHexValue) {
    ...
    const rgbFullText = span.textContent || "";
    ...
    copyToClipboard(textToCopy)
      .then(() => {
        if (currentCellElement) {
          showCopiedFeedback(currentCellElement);
        }
...
export function setupTooltipEventListeners(): void {
  const tooltipHexValue = document.querySelector(
    "#palette-tooltip #tooltip-hex span",
  ) as HTMLElement | null;
  const tooltipHslValue = document.querySelector(
...
export function handleTooltipHotkeysCopy(event: KeyboardEvent): void {
  const tooltipHexValue = document.querySelector(
    "#palette-tooltip #tooltip-hex span",
  ) as HTMLElement | null;
  const tooltipHslValue = document.querySelector(
  ...
  if (
    ...
    if (event.key === "1" || event.key === "2" || event.key === "3") {
      ...
      switch (event.key) {
        case "1": // HEX
          textToCopy = tooltipHexValue?.textContent || null;
          copyTypeForLog = "HEX";
          break;
        case "2": // HSL
          textToCopy = tooltipHslValue?.textContent || null;
          copyTypeForLog = "HSL";
          break;
        case "3": // RGB
