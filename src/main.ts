...
interface DOMElements {
  ...
  configToggleButton: HTMLElement | null;
  configModal: HTMLElement | null;
  closeModalButton: HTMLElement | null;
  editPaletteButton: HTMLElement | null;
  popoutCloseButton: HTMLElement | null;
  ...
  colorPickToggleButton: HTMLElement | null;
  colorPickerModal: HTMLElement | null;
  exportPngButton: HTMLElement | null;
  popoutUpdateButton: HTMLElement | null;
  popoutPaletteInput: HTMLTextAreaElement | null;
  saveStateButton: HTMLElement | null;
  resetStateButton: HTMLElement | null;
  exportPaletteButton: HTMLElement | null;
  importPaletteButton: HTMLElement | null;
  importPaletteFileInput: HTMLInputElement | null;
}

// DOM elements
let elements: DOMElements = {} as DOMElements;
let canvasManager: CanvasInteractionManager | null = null;
...
function setupConfigModalListeners(): void {
...
function setupPaletteEditorListeners(): void {
...
function setupStateManagementListeners(): void {
  ...
  try {
    const state = {
      ...
      paletteOffsetX: canvasManager
        ? (canvasManager as any).offsetX
        : INITIAL_OFFSETS.PALETTE_OFFSET_X,
      paletteOffsetY: canvasManager
        ? (canvasManager as any).offsetY
        : INITIAL_OFFSETS.PALETTE_OFFSET_Y,
      scale: canvasManager ? (canvasManager as any).scale : 1,
    };
...
function loadState(): void {
  try {
    ...
    if (state.sourceGridData && Array.isArray(state.sourceGridData)) {
      ...
      setCurrentGridData([...sourceGridData.map((row: string[]) => [...row])]);
    }
...
function updateUIControls(): void {
...
function showPopoutStatus(message: string, isError: boolean = false): void {
...
// Export palette data
function exportPaletteData(): void {
  try {
...
function handlePaletteImport(event: Event): void {
  const target = event.target as HTMLInputElement;
